...
float Kp = 3;   // Aumentado: el error máximo es 200, 200*0.8 = 160 de corrección
float Ki = 0.1;  
float Kd = 2;   
...
float setpoint = 300;
int VELOCIDAD_BASE = 70; // Sube esto si el mínimo es 60 (2r 3az, 4r 5n)
...
void loop() {
  ...
  if (DT >= 0.01) {
    ...
    // Opcional: Debug para ver en qué estado está el robot
    /*
    Serial.print("Estado: ");
    if(estadoActual == SIGUIENDO_LINEA) Serial.println("LINEA");
    if(estadoActual == MODO_INERCIA) Serial.println("INERCIA");
    if(estadoActual == MODO_RECUPERACION) Serial.println("RECUPERANDO");
    */
  }
}
